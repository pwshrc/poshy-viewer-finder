...
function Get-Viewer-VSCodeInsiders {
    param(
    ...
    Process {
        ...
        if ($alwaysDisqualifyingModifiers -contains $true) {
            if ($DebugPreference -eq "Continue") {
                Write-Debug "Get-Viewer-VSCodeInsiders: Always-disqualifying modifiers used. Args: $($PSBoundParameters | ConvertTo-Json)"
            }
            return
        }
        ...
        if ($OnlyIfHosting -and (-not $Env:VSCODE_IPC_HOOK_CLI)) {
            Write-Debug "Get-Viewer-VSCodeInsiders: Not hosting, so not using."
            return
        }
        if ($StayInTerminal -and (-not $Env:VSCODE_IPC_HOOK_CLI)) {
            Write-Debug "Get-Viewer-VSCodeInsiders: Can't stay in terminal when not hosting, so not using."
            return
        ...
        [string] $VSCode_Insiders_bin = (Search-CommandPathMemoized "code-insiders" "code_insiders")
        if ($VSCode_Insiders_bin) {
            [string] $invocationSignature = (Get-InvocationSignature $VSCode_Insiders_bin @argsToUse)
            $argsToUse = [System.Collections.Immutable.ImmutableList]::Create($argsToUse)
            [PSCustomObject]@{
                Id = "code_insiders#${invocationSignature}"; Name = "VS Code Insiders"; Bin = $VSCode_Insiders_bin; BinArgs = $argsToUse; AcceptsInputPiped = $false;
            ...
            Write-Debug "Get-Viewer-VSCode: No VS Code Insiders binary ('code-insiders') found."
        }
    }
}
